...
use crate::mapper::mappers::mmc1::board::Board;
use crate::mapper::mappers::mmc1::shift_register::{ShiftRegister, ShiftStatus};
...
const LAYOUT: Layout = Layout::builder()
    ...
    .prg_layout_index(3)
    .prg_layout(&[
        PrgWindow::new(0x6000, 0x7FFF,  8 * KIBIBYTE, Prg::RAM_OR_ABSENT).switchable(P).read_write_status(RS0, WS0),
        PrgWindow::new(0x8000, 0xFFFF, 32 * KIBIBYTE, Prg::ROM).switchable(Q),
    ])
    // Same as above.
    .prg_layout(&[
        PrgWindow::new(0x6000, 0x7FFF,  8 * KIBIBYTE, Prg::RAM_OR_ABSENT).switchable(P).read_write_status(RS0, WS0),
    ...
    .prg_layout(&[
        PrgWindow::new(0x6000, 0x7FFF,  8 * KIBIBYTE, Prg::RAM_OR_ABSENT).switchable(P).read_write_status(RS0, WS0),
        PrgWindow::new(0x8000, 0xBFFF, 16 * KIBIBYTE, Prg::ROM).fixed_number(0),
        PrgWindow::new(0xC000, 0xFFFF, 16 * KIBIBYTE, Prg::ROM).switchable(Q),
    ])
    .prg_layout(&[
        PrgWindow::new(0x6000, 0x7FFF,  8 * KIBIBYTE, Prg::RAM_OR_ABSENT).switchable(P).read_write_status(RS0, WS0),
    ...
    .chr_layout(&[
        ChrWindow::new(0x0000, 0x0FFF, 4 * KIBIBYTE, Chr::ROM_OR_RAM).switchable(C),
        ChrWindow::new(0x1000, 0x1FFF, 4 * KIBIBYTE, Chr::ROM_OR_RAM).switchable(D),
    ])
